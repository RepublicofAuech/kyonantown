...
@bot.tree.command(name="server_icon", description="指定された鯖IDのアイコンを貼ります。")
@app_commands.describe(server_id="鯖ID")
async def server_icon(interaction: discord.Interaction, server_id: str):
    try:
        ...
        if guild:
            if guild.icon:
                ...
                await interaction.response.send_message("指定された鯖にはアイコンが設定されていません")
        else:
            await interaction.response.send_message("指定された鯖が見つかりませんでした")
    except Exception as e:
